...
namespace x86
{
  ...
  struct operand
  {
    ...
    operand(const ir::immediate imm) : value(immediate{.value = imm}) {}
    constexpr explicit operand(sized_register r) : value(r) {}
  ...
  constexpr auto EAX =  operand(operand::sized_register{reg::rax, bit_width::dword});
  constexpr auto AL =   operand(operand::sized_register{reg::rax, bit_width::byte});
  constexpr auto R10D = operand(operand::sized_register{reg::r10, bit_width::dword});
  constexpr auto R11D = operand(operand::sized_register{reg::r11, bit_width::dword});
  constexpr auto RBP =  operand(operand::sized_register{reg::rbp, bit_width::qword});
  constexpr auto EDI =  operand(operand::sized_register{reg::rdi, bit_width::dword});
  constexpr auto ESI =  operand(operand::sized_register{reg::rsi, bit_width::dword});
  constexpr auto EDX =  operand(operand::sized_register{reg::rdx, bit_width::dword});
}  // namespace x86
...
inline std::string x86::operand::to_string() const { return std::format("kkk"); }
